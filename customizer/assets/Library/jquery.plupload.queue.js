		...
		if (e) {
			this.each(function() {
				...
				j.bind("UploadFile", function(l, m) {
					//$('.loaderSection, #uploading-image').show();
				...
				j.bind("Error", function(l, o) {
					...
					if (m) {
						...
						if (o.code == plupload.FILE_SIZE_ERROR) {
							alert(a("Please upload Image Below 10MB.\nYour Image is to large :") + m.name)
						}
				...
				j.bind("FileUploaded", function(l, d, m) {
					response = m.response;
					var n = '';
					n = response.slice(-6);
					if (n == '.Error') {
						var data = response.replace('Error', '');
						jq('.uploadimg-content').css('display', 'none');
						alert(data);
					} else {
						jq('.uploadimg-content').css('display', 'none');
						...
						if (jq(window).width() > 959) {
							if (jq('#uWrapper ul li').length == 0)
								jq('<li><ul class="upInnerUl"><li class="upInnerLi"><a href="javascript:void(0);"><img width="60" height="56" alt="upload" data-src="assets/' + response + '" data-src-retina="assets/' + response + '" src="assets/Skin/images/Spinning-line.gif"></a></li></ul></li>').appendTo('#uWrapper ul');
							else {
								if (jq('#uWrapper > ul > li:last').find('ul li').length < opc_Show_Img_No) {
									jq('#uWrapper > ul > li:last ul').append('<li class="upInnerLi"><a href="javascript:void(0);"><img width="60" height="56" alt="upload"  data-src="assets/' + response + '" data-src-retina="assets/' + response + '" src="assets/Skin/images/Spinning-line.gif"></a>');
								} else {
									jq('#uWrapper > ul').append('<li><ul class="upInnerUl"><li class="upInnerLi"><a href="javascript:void(0);"><img width="60" height="56" alt="upload"  data-src="assets/' + response + '" data-src-retina="assets/' + response + '" src="assets/Skin/images/Spinning-line.gif"></a></li></ul></li>');
								}
							}
						}else if (jq(window).width() < 960){
							jq('#uWrapper > ul').append('<li><a href="javascript:void(0);"><img width="60" height="56" alt="upload" data-src="assets/' + response + '" data-src-retina="assets/' + response + '" src="assets/Skin/images/Spinning-line.gif"></a>');
						}

							
						jq('.upload-wrapper .bx-paging').css('display', 'block');
						responsive_slide('opc_upload_Slider', 'uWrapper');
						
						jq("#uWrapper  img").unveil(300);
					}
					h(m)
					opc_Hide_Loader();
				});
